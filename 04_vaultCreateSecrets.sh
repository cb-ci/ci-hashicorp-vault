...
kubectl exec -n $NAMESPACE_VAULT vault-0 -- sh -c "vault kv  put  $SECRET_PATH_KV1/cloudbees-login username=admin password=admin"
...
kubectl exec -n $NAMESPACE_VAULT vault-0 -- sh -c "vault kv  put  $SECRET_PATH_KV2/cloudbees-login username=admin password=admin"
...
kubectl exec -n $NAMESPACE_VAULT vault-0 -- sh -c "vault kv  put  $SECRET_PATH_TEST/cloudbees-login username=admin password=admin"
